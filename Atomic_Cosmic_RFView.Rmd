...
<script>
// Enhanced navigation with left TOC (topics/subtopics) and right TOC (chapters)
(function() {
  'use strict';
  
  console.log('=== RF Navigation System Starting ===');
  ...
  function init() {
    ...
    waitForTOC(function() {
      ...
      createRightChapterNav();
      addTOCMinimizeButton();
      initializeMobileView();
      updateMainContainerMargins();
    });
  }
  
  function initializeMobileView() {
    // On mobile, start with navigation minimized
    const isMobile = window.innerWidth <= 768;
    if (isMobile) {
      const leftNav = document.querySelector('.chapters-nav');
      const rightTOC = document.getElementById('TOC');
      
      if (leftNav && !leftNav.classList.contains('minimized')) {
        leftNav.classList.add('minimized');
      }
      if (rightTOC && !rightTOC.classList.contains('minimized')) {
        rightTOC.classList.add('minimized');
      }
      
      console.log('Mobile view initialized - navigation minimized');
    }
  }
  
  function addTOCMinimizeButton() {
    console.log('Adding TOC minimize button...');
    const toc = document.getElementById('TOC');
    if (!toc) return;
    
    // Add wrapper for TOC with button
    toc.classList.add('toc-with-toggle');
    
    // Create toggle button
    const toggleBtn = document.createElement('button');
    toggleBtn.className = 'toc-toggle-btn';
    toggleBtn.title = 'Minimize TOC';
    toggleBtn.innerHTML = '→';
    toggleBtn.onclick = function() {
      toc.classList.toggle('minimized');
      toggleBtn.innerHTML = toc.classList.contains('minimized') ? '←' : '→';
      updateMainContainerMargins();
    };
    
    toc.insertBefore(toggleBtn, toc.firstChild);
    
    // Make TOC collapsible - collapse non-active items
    makeRightTOCCollapsible();
    
    console.log('✓ TOC minimize button added');
  }
  
  function makeRightTOCCollapsible() {
    const toc = document.getElementById('TOC');
    if (!toc) return;
    
    // Add expand/collapse icons to TOC items with children
    ...
    tocItems.forEach(function(li) {
      ...
      if (ul) {
        ...
        if (link) {
          ...
          icon.className = 'toc-expand-icon';
          icon.textContent = '▶';
          icon.onclick = function(e) {
            e.stopPropagation();
            e.preventDefault();
            li.classList.toggle('toc-expanded');
            icon.textContent = li.classList.contains('toc-expanded') ? '▼' : '▶';
          };
    ...
    // Expand active items on scroll
    window.addEventListener('scroll', function() {
      expandActiveTOCItems();
    });
    
    expandActiveTOCItems();
  }
  
  function expandActiveTOCItems() {
    const toc = document.getElementById('TOC');
    if (!toc) return;
    
    // First collapse all
    toc.querySelectorAll('.toc-collapsible').forEach(function(li) {
      li.classList.remove('toc-expanded');
      const icon = li.querySelector('.toc-expand-icon');
      if (icon) icon.textContent = '▶';
    });
    
    // Find and expand active item's parents
    const activeLink = toc.querySelector('a.active, .tocify-focus');
    if (activeLink) {
      let parent = activeLink.closest('.toc-collapsible');
      while (parent) {
        parent.classList.add('toc-expanded');
        const icon = parent.querySelector('.toc-expand-icon');
        if (icon) icon.textContent = '▼';
        parent = parent.parentElement.closest('.toc-collapsible');
      }
    }
  }
  
  function updateMainContainerMargins() {
    // Check if mobile/tablet
    const isMobile = window.innerWidth <= 768;
    const isTablet = window.innerWidth > 768 && window.innerWidth <= 1200;
    
    // Find all possible main container elements
    const selectors = ['.main-container', '.row-fluid .span9', '#main', '.container-fluid > .row-fluid'];
    let mainContainers = [];
    
    selectors.forEach(function(selector) {
      const elements = document.querySelectorAll(selector);
      elements.forEach(function(el) {
        if (!mainContainers.includes(el)) {
          mainContainers.push(el);
        }
      });
    });
    ...
    // On mobile/tablet, no margins needed
    if (isMobile || isTablet) {
      ...
      console.log('Mobile/Tablet mode: Full width applied');
      return;
    }
    ...
    const leftMinimized = leftNav && leftNav.classList.contains('minimized');
    const rightMinimized = rightTOC && rightTOC.classList.contains('minimized');
    
    // Calculate margins based on minimized state
    const leftMargin = leftMinimized ? '70px' : '320px';
    const rightMargin = rightMinimized ? '70px' : '290px';
    
    // Calculate width dynamically
    const leftValue = leftMinimized ? 70 : 320;
    const rightValue = rightMinimized ? 70 : 290;
    const calcWidth = 'calc(100% - ' + leftValue + 'px - ' + rightValue + 'px)';
    
    // Apply to all main containers
    mainContainers.forEach(function(container) {
      container.style.setProperty('margin-left', leftMargin, 'important');
      container.style.setProperty('margin-right', rightMargin, 'important');
      container.style.setProperty('width', calcWidth, 'important');
      container.style.setProperty('max-width', 'none', 'important');
    });
    
    console.log('Desktop mode: margins=' + leftMargin + '/' + rightMargin + ', width=' + calcWidth + ' (applied to ' + mainContainers.length + ' containers)');
  }
  ...
  function waitForTOC(callback) {
    ...
    const check = setInterval(function() {
      attempts++;
      const toc = document.getElementById('TOC');
      const tocItems = toc ? toc.querySelectorAll('li').length : 0;
      
      if (tocItems > 5) {
        console.log('✓ TOC ready with ' + tocItems + ' items');
        clearInterval(check);
        callback();
      } else if (attempts >= maxAttempts) {
        console.error('✗ TOC not found after ' + attempts + ' attempts');
        clearInterval(check);
      }
    }, 200);
  }
  
  function createRightChapterNav() {
    console.log('\n=== Creating Right Chapter Navigation ===');
    
    // Create right navigation container
    const navDiv = document.createElement('div');
    navDiv.className = 'chapters-nav';
    navDiv.innerHTML = '<button class="toggle-btn" title="Minimize">←</button><div class="nav-content"><h4>Chapters</h4><ul id="chapters-list"></ul></div>';
    
    const toggleBtn = navDiv.querySelector('.toggle-btn');
    toggleBtn.onclick = function() {
      navDiv.classList.toggle('minimized');
      updateMainContainerMargins();
    };
    
    const chaptersList = navDiv.querySelector('#chapters-list');
    
    // Build chapter navigation from h3 and h4 headings
    const h1Sections = document.querySelectorAll('.section.level1');
    let chapterCount = 0;
    
    h1Sections.forEach(function(h1Section) {
      const h1 = h1Section.querySelector('h1');
      if (!h1) return;
      
      const h1Text = h1.textContent.replace(/^\d+\s*/, '').trim();
      
      // Add main topic
      const h1Li = document.createElement('li');
      h1Li.className = 'chapter-h1 collapsible-item';
      const h1Link = document.createElement('a');
      h1Link.href = '#' + h1Section.id;
      h1Link.innerHTML = '<span class="expand-icon">▶</span> ' + h1Text;
      h1Link.onclick = function(e) {
        e.preventDefault();
        toggleCollapsible(h1Li);
        scrollToElement(h1Section.id);
        setActiveChapter(h1Link);
      };
      h1Li.appendChild(h1Link);
      
      // Add subtopics (h2)
      const h2Sections = h1Section.querySelectorAll('.section.level2');
      if (h2Sections.length > 0) {
        const h2Ul = document.createElement('ul');
        h2Ul.className = 'collapsible-content';
        
        h2Sections.forEach(function(h2Section) {
          const h2 = h2Section.querySelector('h2');
          if (!h2) return;
          
          const h2Text = h2.textContent.replace(/^\d+\.\d+\s*/, '').trim();
          const h2Li = document.createElement('li');
          h2Li.className = 'chapter-h2 collapsible-item';
          const h2Link = document.createElement('a');
          h2Link.href = '#' + h2Section.id;
          h2Link.innerHTML = '<span class="expand-icon">▶</span> ' + h2Text;
          h2Link.onclick = function(e) {
            e.preventDefault();
            toggleCollapsible(h2Li);
            scrollToElement(h2Section.id);
            setActiveChapter(h2Link);
          };
          h2Li.appendChild(h2Link);
          
          // Add chapters (h3)
          const h3Sections = h2Section.querySelectorAll('.section.level3');
          if (h3Sections.length > 0) {
            const h3Ul = document.createElement('ul');
            h3Ul.className = 'collapsible-content';
            
            h3Sections.forEach(function(h3Section) {
              const h3 = h3Section.querySelector('h3');
              if (!h3) return;
              
              const h3Text = h3.textContent.replace(/^\d+\.\d+\.\d+\s*/, '').trim();
              const h3Li = document.createElement('li');
              h3Li.className = 'chapter-h3';
              const h3Link = document.createElement('a');
              h3Link.href = '#' + h3Section.id;
              h3Link.textContent = h3Text;
              h3Link.onclick = function(e) {
                e.preventDefault();
                scrollToElement(h3Section.id);
                setActiveChapter(h3Link);
              };
        ...
        if (h2Ul.children.length > 0) {
          h1Li.appendChild(h2Ul);
        }
      }
      
      chaptersList.appendChild(h1Li);
    });
    
    document.body.appendChild(navDiv);
    
    console.log('✓ Created chapter navigation with ' + chapterCount + ' chapters');
  ...
  function collapseAllItems() {
    document.querySelectorAll('.chapters-nav .collapsible-item').forEach(function(item) {
  ...
  function expandActiveItems() {
    ...
    const activeLink = document.querySelector('.chapters-nav a.active');
    if (!activeLink) return;
    
    let parent = activeLink.closest('.collapsible-item');
    while (parent) {
      parent.classList.add('expanded');
      const icon = parent.querySelector('.expand-icon');
      if (icon) icon.textContent = '▼';
      parent = parent.parentElement.closest('.collapsible-item');
    }
  }
  ...
  function setActiveChapter(link) {
    document.querySelectorAll('.chapters-nav a').forEach(function(a) {
  ...
  function updateActiveChapter() {
    ...
    document.querySelectorAll('.section.level3, .section.level2, .section.level1').forEach(function(section) {
    ...
    if (activeSection) {
      ...
      let hasChange = false;
      document.querySelectorAll('.chapters-nav a').forEach(function(link) {
